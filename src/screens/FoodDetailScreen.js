...
export default function FoodDetailScreen({ navigation, route }) {
  ...
  const { addFood, profile, favorites, toggleFavorite } = useDiary();
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: c.pageBg }]} edges={['top', 'bottom']}>
        ...
        <View style={[styles.heroCard, cardShadow, { borderColor: isDark ? c.border : 'rgba(0,112,224,0.14)' }]}>
          ...
          >
            ...
            <View style={styles.macroRow}>
              <MacroChip icon={Wheat} label="Carbs" value={`${scaled.carbs}g`} color={colors.carbs} theme={c} isDark={isDark} />
              <MacroChip icon={Beef} label="Protein" value={`${scaled.protein}g`} color={colors.protein} theme={c} isDark={isDark} />
              <MacroChip icon={Droplet} label="Fat" value={`${scaled.fat}g`} color={colors.fat} theme={c} isDark={isDark} />
            </View>
          </LinearGradient>
        </View>
      ...
      <View style={[styles.footer, { borderTopColor: c.border, backgroundColor: c.pageBg }]}>
        ...
        <Pressable onPress={log} style={({ pressed }) => [styles.cta, { opacity: pressed ? 0.9 : 1 }]}>
...
function MacroChip({ icon: Icon, label, value, color, theme, isDark }) {
  return (
    <View style={[styles.macroChip, { backgroundColor: isDark ? '#1C1C1E' : `${color}14`, borderColor: `${color}44` }]}>
      <Icon size={14} color={color} />
      <Text style={[styles.macroVal, { color, fontFamily: snPro('800') }]}>{value}</Text>
      <Text style={[styles.macroLbl, { color: theme.muted, fontFamily: snPro('600') }]}>{label}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  macroVal: { fontSize: 15, marginTop: 4 },
  macroLbl: { fontSize: 10, marginTop: 2 },
  card: {
    borderRadius: 16,
    borderWidth: 1,
    padding: 16,
